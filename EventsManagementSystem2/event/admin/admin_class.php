...
Class Action {
	...
	function login(){
		extract($_POST);
		$username = isset($username) ? trim(strtolower($username)) : '';
		if($username === 'admin'){
	...
	function save_user(){
		extract($_POST);
		$username = isset($username) ? trim(strtolower($username)) : '';
		$type = isset($type) ? (int)$type : 2;
		if($type === 1 && $username !== 'admin@event.com'){
			return 4;
		}
		$data = " name = '$name' ";
	...
	function save_book(){
		...
		// Server-side validation: require all fields and valid formats
		$venue_id = isset($venue_id) ? trim($venue_id) : '';
		$name = isset($name) ? trim($name) : '';
		$address = isset($address) ? trim($address) : '';
		$email = isset($email) ? trim($email) : '';
		$contact = isset($contact) ? trim($contact) : '';
		$schedule = isset($schedule) ? trim($schedule) : '';
		$duration = isset($duration) ? trim($duration) : '';

		if(empty($venue_id) || empty($name) || empty($address) || empty($email) || empty($contact) || empty($schedule) || empty($duration)){
			// 2 = missing required fields
			return 2;
		}
		if(!ctype_digit($venue_id)){
			// 6 = invalid venue or schedule
			return 6;
		}
		$venueExists = $this->db->query("SELECT id FROM venue WHERE id = '".$this->db->real_escape_string($venue_id)."' LIMIT 1");
		if(!$venueExists || $venueExists->num_rows <= 0){
			return 6;
		}
		// validate email
		...
		// contact must be digits only and between 7 and 15 characters
		if(!preg_match('/^\\d{7,15}$/', $contact)){
			// 3 = invalid contact
			return 3;
		}
		$parsedSchedule = strtotime($schedule);
		if($parsedSchedule === false || $parsedSchedule < time()){
			return 6;
		}
	...
	function save_register(){
		extract($_POST);
		// require logged-in student to RSVP
		if(!isset($_SESSION['login_id']) || $_SESSION['login_type'] != 3){
			// 5 = not authenticated
			return 5;
		}
		$event_id = isset($event_id) ? trim($event_id) : '';
		$email = isset($_SESSION['login_username']) ? trim($_SESSION['login_username']) : '';
		$name = isset($_SESSION['login_name']) ? trim($_SESSION['login_name']) : '';
		$address = isset($address) ? trim($address) : '';
		$contact = isset($contact) ? trim($contact) : '';
		$require_details = isset($require_details) ? (int)$require_details : 0;

		if($event_id === '' || !ctype_digit($event_id)){
			// 4 = invalid event id
			return 4;
		}
		$eventExists = $this->db->query("SELECT id FROM events WHERE id = '".$this->db->real_escape_string($event_id)."' LIMIT 1");
		if(!$eventExists || $eventExists->num_rows <= 0){
			return 4;
		}
		if($require_details === 1 && (empty($address) || empty($contact))){
			// 6 = missing required fields
			return 6;
		}
		if(!empty($contact) && !preg_match('/^\\d{7,15}$/', $contact)){
			// 3 = invalid contact
			return 3;
		}
		...
		if(empty($id)){
			$chk = $this->db->query("SELECT * FROM audience where event_id = '$event_id' and email = '$email' ")->num_rows;
			if($chk > 0){
				return 2;
			}
			$save = $this->db->query("INSERT INTO audience set ".$data);
			if(!$save && (int)$this->db->errno === 1062){
				return 2;
			}
		}else{
			$id = (int)$id;
			$chk = $this->db->query("SELECT id FROM audience where event_id = '$event_id' and email = '$email' and id != '$id' ")->num_rows;
			if($chk > 0){
				return 2;
			}
	...
	function save_event(){
		extract($_POST);
		$data = " event = '$event' ";
		$data .= ", category = '".(isset($category) && $category != '' ? $category : 'General')."' ";
		$data .= ",venue_id = '$venue_id' ";
